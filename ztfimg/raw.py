...
class RawQuadrant( Quadrant ):
    ...
    def get_data(self,
        ...
        corr_pocket: bool or dict
            ...
            provides multiplicative factors to multiply default parameters
            with, the dict is (str: float) and must contains the following
            keys: alpha, cmax, beta, nmax. See ztfsensors.pocket.PocketModel
            for details.
        ...
        if corr_pocket:
            ...
            if isinstance(corr_pocket, dict):
                # pocket effect parameters multiplicative factors are given,
                # ensure the expected values are here
                if missing := (set(("cmax", "nmax", "alpha", "beta")) - corr_pocket.keys()):
                    raise ValueError(f"pocket effect correction factors are missing: {', '.join(missing)}")
                if extra := (set(corr_pocket.keys()) - set(("cmax", "nmax", "alpha", "beta"))):
                    warnings.warn(f"ignoring extra factors given to corr_pocket: {', '.join(extra)}")

                pocket_config = {k: v * float(corr_pocket[k]) for k, v in pocket_config.items()}
